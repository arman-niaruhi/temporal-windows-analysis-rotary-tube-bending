...
def visualize_attention_single_angle(experiment_id, angle_idx, model, dfs):
    ...
    X_sample = sample_df.values[np.newaxis, :, :]  # shape: (1, seq_len, num_features)
    ...
    preds = preds.cpu().numpy()[0]                # (num_angles, output_size)
    attn_weights = attn_weights.cpu().numpy()[0]  # (num_angles, seq_len)
    ...
    # --- Subplot 2: Attention weights (line) ---
    ...
    x_axis = sample_df.index
    y_axis = np.arange(attn_weights.shape[0])  # angles
    ...
    plt.yticks(y_axis + 0.5, y_axis)  # center tick labels
...
def plot_most_accurate_window_angle_lstm(sample_idx, X, Y, model, num_angles, num_features, patch_size, angle_idx=0):
    ...
    for w in range(num_patches):
        ...
        x_tensor = torch.tensor(x_patch_only[np.newaxis, :, :], dtype=torch.float32)
        with torch.no_grad():
            y_hat, _ = model(x_tensor)  # <-- unpack tuple
            y_hat = y_hat.cpu().numpy()[0, angle_idx]  # (num_outputs,) or scalar
    ...
    x_tensor = torch.tensor(x_input[np.newaxis, :, :], dtype=torch.float32)
    with torch.no_grad():
        preds, attn_weights = model(x_tensor)  # <-- unpack tuple
        preds = preds.cpu().numpy()[0]  # shape: (num_angles, num_outputs)
