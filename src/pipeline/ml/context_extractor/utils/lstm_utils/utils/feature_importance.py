...
def compute_attention_importance(model, val_loader, feature_names, device, output_dir="images/04_feature_importance"):
    """
    ...
    if all_attention_weights:
        ...
        feature_importance = np.maximum(feature_importance, 0.001)  # Ensure positive values
    ...
    bars = plt.barh(range(len(top_df)), top_df['Attention_Importance'], color=colors, alpha=0.8)
    ...
    plt.title(f'Top {top_features} Most Important Features\nLSTM Attention-Based Feature Importance', 
    ...
    for i, (bar, val) in enumerate(zip(bars, top_df['Attention_Importance'])):
        plt.text(val + 0.001, i, f'{val:.4f}', 
                va='center', ha='left', fontsize=10, fontweight='bold',
                bbox=dict(boxstyle="round,pad=0.2", facecolor="white", alpha=0.8))

    plt.tight_layout()
    plot_paths['feature_importance'] = output_dir / "01_feature_importance_bars.png"
    plt.savefig(plot_paths['feature_importance'], dpi=150, bbox_inches='tight', facecolor='white')
    plt.close()
    ...
    if len(timestep_attention) > 1:
        ...
        plt.plot(x_vals, timestep_attention, 'o-', color='#2E86AB', 
                linewidth=3, markersize=8, markerfacecolor='#A23B72', 
                markeredgecolor='#6D214F', markeredgewidth=2, label='Attention Weights')
        ...
        plt.scatter(peaks, timestep_attention[peaks], color='#F18F01', s=150, 
                   zorder=5, label='High Attention Peaks', edgecolors='#C73E1D', linewidth=2)
        
        # Add trend line
        if len(timestep_attention) > 3:
            z = np.polyfit(x_vals, timestep_attention, 2)
            p = np.poly1d(z)
            plt.plot(x_vals, p(x_vals), "--", color='#C73E1D', linewidth=2, alpha=0.7, label='Trend Line')
    ...
    plt.savefig(plot_paths['time_distribution'], dpi=150, bbox_inches='tight', facecolor='white')
    plt.close()
    ...
    if len(timestep_attention) > 1:
        ...
        feature_labels = [feature_names[i] if i < len(feature_names) else f"Feature {i}" for i in feature_indices]
        plt.yticks(feature_indices, feature_labels, fontsize=9)
        
        plt.tight_layout()
        plot_paths['heatmap'] = output_dir / "03_attention_heatmap.png"
        plt.savefig(plot_paths['heatmap'], dpi=150, bbox_inches='tight', facecolor='white')
        plt.close()
    ...
    if len(timestep_attention) > 1:
        ...
        plt.plot(np.arange(len(timestep_attention)), cumulative_attention, 
                'g-', linewidth=4, alpha=0.8, label='Cumulative Attention')
        ...
        for i, (threshold, color) in enumerate(zip(thresholds, threshold_colors)):
            ...
            if len(idx) > 0:
                step = idx[0]
                plt.axvline(x=step, color=color, linestyle='--', alpha=0.8, linewidth=2,
                           label=f'{threshold*100:.0f}% attention by step {step}')
                
                # Add annotation
                plt.annotate(f'{threshold*100:.0f}%', xy=(step, threshold), 
                            xytext=(10, 20+i*30), textcoords='offset points',
                            fontsize=11, fontweight='bold', color=color,
                            arrowprops=dict(arrowstyle='->', color=color, alpha=0.7))
        
        plt.xlabel('Time Step', fontsize=13, fontweight='bold')
        plt.ylabel('Cumulative Attention Fraction', fontsize=13, fontweight='bold')
        plt.title('Cumulative Attention Distribution\nHow Quickly the Model Focuses Its Attention', 
                  fontsize=15, fontweight='bold', pad=20)
        ...
        plt.savefig(plot_paths['cumulative'], dpi=150, bbox_inches='tight', facecolor='white')
        plt.close()
    ...
    if len(timestep_attention) > 1:
        ...
        summary_text.append(f"• Mean Attention Weight: {np.mean(timestep_attention):.4f}")
        summary_text.append(f"• Attention Std Dev: {np.std(timestep_attention):.4f}")
        summary_text.append(f"• Attention Range: {np.min(timestep_attention):.4f} - {np.max(timestep_attention):.4f}")
        summary_text.append(f"• Peak Attention at Step: {np.argmax(timestep_attention)}")
        summary_text.append("")
    ...
    plt.text(0.05, 0.95, summary_str, transform=plt.gca().transAxes, fontsize=12,
             fontfamily='monospace', verticalalignment='top', linespacing=1.5,
             bbox=dict(boxstyle="round,pad=1.0", facecolor="lightblue", alpha=0.9,
                      edgecolor="navy", linewidth=2))
    
    plt.title('Attention Analysis Summary Dashboard', fontsize=16, fontweight='bold', pad=20)
    plt.tight_layout()
    ...
    plt.semilogy(ranks, importance_values, 's-', color='#6A0572', 
                linewidth=2, markersize=6, alpha=0.8, label='Feature Importance')
    
    # Highlight top features
    top_n = min(10, len(importance_df))
    plt.scatter(ranks[:top_n], importance_values[:top_n], color='#FF6B6B', s=100,
               zorder=5, label=f'Top {top_n} Features', edgecolors='darkred', linewidth=2)
    
    plt.xlabel('Feature Rank', fontsize=13, fontweight='bold')
    plt.ylabel('Attention Importance (Log Scale)', fontsize=13, fontweight='bold')
    plt.title('Feature Importance Distribution by Rank\nIdentifying the Most Influential Features', 
              fontsize=15, fontweight='bold', pad=20)
    plt.legend(fontsize=11)
    ...
    plt.savefig(plot_paths['rank_distribution'], dpi=150, bbox_inches='tight', facecolor='white')
    plt.close()
...
def compute_ablation_importance(model, val_loader, feature_names, device, output_dir="images/04_feature_importance"):
    """
    ...
    importance_df = pd.DataFrame({
        'Feature': feature_names,
        'Ablation_Importance': importance_scores
    ...
    bars = ax.barh(importance_df['Feature'], importance_df['Ablation_Importance'], color=colors)
...
def compute_permutation_importance(model, val_loader, feature_names, device, output_dir="images/04_feature_importance"):
    """
    ...
    bars = ax.barh(importance_df['Feature'], importance_df['Permutation_Importance'], color=colors)
...
def analyze_feature_importance(model, X_val, val_loader, feature_names, device):
    ...
    """
    print("\n" + "="*80)
    print("COMPUTING FEATURE IMPORTANCE ANALYSIS FOR LSTM")
    print("="*80)
    ...
    try:
        ...
        attn_df, attn_path, attn_data = compute_attention_importance(model, val_loader, feature_names, device)
    ...
    try:
        ...
        perm_df, perm_path = compute_permutation_importance(model, val_loader, feature_names, device)
    ...
    try:
        ...
        ablation_df, ablation_path = compute_ablation_importance(model, val_loader, feature_names, device)
    ...
    # Print results from all successful methods
    print("\n" + "="*80)
    for method, df in importance_dfs.items():
        if df is not None:
            print(f"\nTOP 10 FEATURES BY {method.upper()} IMPORTANCE")
            print("="*80)
            for idx, (i, row) in enumerate(df.head(10).iterrows(), 1):
                col_name = df.columns[1]  # Get the importance column name
                print(f"{idx:2d}. {row['Feature']:40s} | {method}: {row[col_name]:.6f}")
            print("="*80)
    
    # Create comparison table if multiple methods succeeded
    successful_methods = [method for method in importance_dfs.keys() if importance_dfs[method] is not None]
    if len(successful_methods) > 1:
        print("\nFEATURE IMPORTANCE RANK COMPARISON")
        print("="*80)
        comparison_data = []
        for feature in feature_names[:10]:  # Top 10 features by first method
            ...
            for method in successful_methods:
                ...
                rank = df[df['Feature'] == feature].index[0] + 1 if feature in df['Feature'].values else len(feature_names)
                row[method] = rank
