...
def training_pipeline(
    ...
    if not os.path.exists(annotation_json_path):
        logger.error(
            f"Annotation JSON file not found: {annotation_json_path}. Please check the provided path. If you did not provide the annotations, please use the annotation.py to do that."
        )

    if not os.path.exists(database_path):
        logger.error(
            f"Database file not found: {database_path}. Please check the provided path. If you did not preprocess the data, please run the preprocessing step first using 'python main.py preprocess'."
        )

    if not os.path.exists(experiment_ids_path):
        logger.error(
            f"Experiment IDs file not found: {experiment_ids_path}. Please check the provided path. If you dont see any expeiment file download the file from github page."
        )

    if label not in ["All", "Clamping", "Bending", "Mandrel Extraction", "De-Clamping"]:
        logger.error(
            f"Invalid label: {label}. Supported labels are 'All', 'Clamping', 'Bending', 'Mandrel Extraction', 'De-Clamping'."
        )

    if pipeline_config is None:
        logger.error(
            "Pipeline configuration is missing. Please provide a valid configuration dictionary."
        )

    if machine_part not in ["machine_and_movement", "movement"]:
        logger.error(
            f"Invalid machine part: {machine_part}. Supported machine parts are 'machine_and_movement' and 'movemenet'."
        )

    logger.info("(Machine Activity Recognition) Starting training pipeline...")
    logger.info(f"Using label: {label}, machine part: {machine_part}")
    ...
    logger.info("Reading and preprocessing the experiments ids ...")
    ...
    if label == "All":
        ...
        sensors_df = classifier_preprocessor.assign_one_label(
            target_label=label
        )  # Clamping, Bending, De-Clamping, Mandrel Extraction
    ...
    if "dataloader_config" not in pipeline_config:
        logger.error(
            "Dataloader configuration is missing in the pipeline configuration."
        )
    ...
    if training_flag:
        ...
        model.train_model(
            ...
            learning_rate=pipeline_config.get("training_config").get(
                "learning_rate", 1e-5
            ),
        ...
        logger.info("Loading the trained model and do inference...")
        state_dict = torch.load(
            f"{model_path}/activity_detector.pth", map_location=device
        )
        logger.info(f"Loaded model from {model_path}/activity_detector.pth")
...
def analyze_features(model, sensors_df, test_loader, device):
    ...
    plot_feature_importance(
        intgrad_importances, None, feature_names, "IntegratedGradients"
    )
